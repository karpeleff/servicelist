...
@section('content')
  ...
  <label >Показания счетчика за  {{$mons}} месяц:<font color="green">{{ $data->counter }}</font></label>
  </div>   
@endisset
     ...
     <div class="form-group">
		<label for="phone" class="col-sm-2 control-label">Показания на конец месяца </label>
		   <div class="col-sm-6">
			<input type="text" class="form-control" id="phone" name="counter"  placeholder="показания счетчика в формате ХХХХХХ (X-цифра)" pattern="\d{6}" required>
